...
function RFQbutton() {
        ...
        const [items,setItems]=useState([{name:'Mayank',id:'1',status:'LIVE',date:'12/10/1999'},{name:'Rohan',id:'2',status:'MODIFY',date:'05/09/2000'},]);
...
//Tonne End
    ...
    return (
        ...
        <div className='entirePage'>
         ...
         {/* middle Column */}
         <div className="column-landing col-midut">
            <div className="rfq-Box ">
                <h3>LIVE RFQs</h3>
                <div className='utl-btn-div1'>
                    {items.map((item)=>(
                        <div id='card' onClick={setIsOpenRfqs}>
                            <p className='placename'>{item.name}</p>
                            <p className='logorfq'>{item.id}</p>
                            <p className='statusRfq' style={{backgroundColor:checkStatus(item)}}>{item.status}</p>
                            <p className='lastDaterfq'><b>Last Date:</b>{item.date}</p>
                        </div>
                    ))}
